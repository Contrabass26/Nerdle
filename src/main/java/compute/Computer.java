...
public class Computer {
    ...
    public static double getEntropy(String guess, List<String> possibilities) {
        ...
        final List<String> finalSubset = subset.subList(0, subset.size() / CONFIG.getOption("subset_size", JsonNode::asInt));
    ...
    public static List<Map.Entry<String, Double>> getPossibilities(Guess... guesses) {
        ...
        for (int equalsPos = 0; equalsPos < charPossibilities.length; equalsPos++) {
            LOGGER.log("Equals position: " + equalsPos, 3);
            ...
            for (int i = 0; i < configs.length; i++) {
                ...
                LOGGER.log(String.format("Config %s of %s: %s", i + 1, configs.length, config), 3);
                ...
                loops[loops.length - 1] = operators -> {
                    ...
                    LOGGER.log("ITERATING " + operatorConfig, 3);
                ...
                loops[0].run();
                LOGGER.log("Finished config %s of %s: %s".formatted(i + 1, configs.length, config), 3);
            }
        }
        LOGGER.log("Collected " + possibilities.size() + " possibilities", 3);
        // Sort and return
        return sortPossibilities(possibilities);
    }

    private static List<Map.Entry<String, Double>> sortPossibilities(List<String> possibilities) {
        LOGGER.log("Sorting...", 3);
        ...
        LOGGER.log("Sorted possibilities", 3);
        return entropyMap.sortedEntryList();
    }

    public static Set<String> getAllPossibilities() {
        // Limitations
        String[] charPossibilities = getBaseCharPossibilities(); // What the character at each index could be
        List<Predicate<String>> predicates = getPredicates(charPossibilities);
        // Iterate through each equals position
